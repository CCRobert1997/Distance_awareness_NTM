...
flags.DEFINE_float('tSNE_loss_weight', 10.0, 'tSNE_loss_weight')
...
def run_ntsm(args):
    ...
    save_dir = os.path.join(base_dir, 'save', 'dataset%s_K%d_RW%0.3f_RS%d_L%0.3ftSNEW%0.3fwithoutOT' %
    ...
    P = utils.seach_prob(train_data, 1e-5, FLAGS.perplexity)
    P = P + np.transpose(P)
    P = P / np.sum(P)  # pij
    ...
    joint_loss = rec_loss_weight * rec_loss + tSNE_loss_weight*tSNE_loss
    ...
    with session as sess:
        ...
        for epoch in range(FLAGS.n_epochs):
            ...
            rec_loss_avg, joint_loss_avg = 0., 0.
            ...
            for batch in tqdm(range(nb_batches)):
                ...
                P_batch = P_batch / np.sum(P_batch)  # pij
                ...
                #print(tSNE_X.shape)
                _, rec_loss_batch, tSNE_loss_batch, joint_loss_batch = \
                    sess.run([trainer, rec_loss, tSNE_loss, joint_loss],
                             feed_dict={doc_word_ph: X, word_embeddings_ph: word_embeddings, tSNE_X: P_batch, rec_loss_weight: FLAGS.rec_loss_weight, tSNE_loss_weight: FLAGS.tSNE_loss_weight})
                #print(P.shape)
                #print(tSNE_X.shape)
