...
flags.DEFINE_integer('n_epochs', 5, 'n_epochs')
...
def run_ntsm(args):
    ...
    with tf.variable_scope('cost_function'):
        ...
        M = tf.sqrt(tf.reduce_sum(tf.square(word_embeddings_ph - topic_embeddings_tf), 1))
    ...
    joint_loss = rec_loss_weight * rec_loss + sh_loss + 100*tSNE_loss
            ...
            for batch in tqdm(range(nb_batches)):
                ...
                _, rec_loss_batch, sh_rec_loss_batch, tSNE_loss_batch, joint_loss_batch = \
                    sess.run([trainer, rec_loss, sh_loss, tSNE_loss, joint_loss],
                             feed_dict={doc_word_ph: X, word_embeddings_ph: word_embeddings, tSNE_X: P_batch, rec_loss_weight: FLAGS.rec_loss_weight})
                #print(P.shape)
                #print(tSNE_X.shape)
