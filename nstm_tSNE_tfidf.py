...
def run_ntsm(args):
    ...
    save_dir = os.path.join(base_dir, 'save', 'dataset%s_K%d_RW%0.3f_RS%d_L%0.3ftfidftSNEW%0.3f' %
    ...
    with open('%s/train_tfidf.pickle' % data_dir, 'rb') as handle:
        unserialized_data_train = pickle.load(handle)

    with open('%s/test_tfidf.pickle' % data_dir, 'rb') as handle:
        unserialized_data_test = pickle.load(handle)
