...
def run_ntsm(args):
    ...
    save_dir = os.path.join(base_dir, 'save', 'dataset%s_K%d_RW%0.3f_RS%d_L%0.3ftSNEwordmove_pretrainedW%0.3f' %
    ...
    with open(data_dir + '/dist_matrix.pk', 'rb') as f:
        load_pairwise_dist = pickle.load(f, encoding="bytes")

    P = utils.seach_prob(train_data, 1e-5, FLAGS.perplexity, "ot", load_pairwise_dist)
    P = P + np.transpose(P)
    P = P / np.sum(P)  # pij
